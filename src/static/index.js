function getData(dataset) {
  ...
  var mapUrl = `https://api.tiles.mapbox.com/v4/{id}/{z}/{x}/{y}.png?access_token=${API_KEY}`;
  let month = 1;
  let year = 2017;
...
if (dataset === 'mint') {
  ...
  d3.json(mintempUrl, function(response) {
    ...
    for (var i = 0; i < response.length; i++) {
      var record = response[i];
      mintempArray.push([record.Lat, record.Long, record['Temp(F)']] * 20);
    }
...
if (dataset === 'maxt') {
  ...
  d3.json(maxtempUrl, function(response) {
    ...
    for (var i = 0; i < response.length; i++) {
      var record = response[i];
      maxtempArray.push([record.Lat, record.Long, record['Temp(F)']] * 20);
    }
